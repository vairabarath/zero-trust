...
func (s *Server) handleUITunnelers(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		out := []uiTunneler{}
		for rows.Next() {
			if t, ok := scanUITunneler(rows); ok {
				out = append(out, t)
			}
		}
	...
	case http.MethodPost:
		...
		if err != nil {
			http.Error(w, "failed to create tunneler", http.StatusBadRequest)
			return
		}
...
func (s *Server) handleUITunneersSubroutes(w http.ResponseWriter, r *http.Request) {
	db, ok := s.uiDB(w)
	if !ok {
		return
	}
	path := strings.TrimPrefix(r.URL.Path, "/api/tunnelers/")
	...
	if len(parts) == 1 {
		if r.Method == http.MethodDelete {
			_ = state.DeleteTunnelerFromDB(db, tunnelerID)
			writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
			return
		}
		...
		row := db.QueryRow(state.Rebind(`SELECT id, name, status, version, hostname, remote_network_id, connector_id, revoked, installed, CAST(last_seen AS TEXT) as last_seen, last_seen_at FROM tunnelers WHERE id = ?`), tunnelerID)
		tunneler, ok := scanUITunneler(row)
		if !ok {
		...
		networkRow := db.QueryRow(state.Rebind(`
			...
			WHERE n.id = ?`), tunneler.RemoteNetworkID)
		...
		logs := []uiConnectorLog{}
		logRows, _ := db.Query(state.Rebind(`SELECT id, timestamp, message FROM tunneler_logs WHERE tunneler_id = ? ORDER BY id ASC`), tunnelerID)
		if logRows != nil {
			for logRows.Next() {
	...
	if len(parts) >= 2 && parts[1] == "revoke" {
		...
		_ = state.RevokeTunnelerInDB(db, tunnelerID)
		nowISO := isoStringNow()
		_, _ = db.Exec(state.Rebind(`INSERT INTO tunneler_logs (tunneler_id, timestamp, message) VALUES (?, ?, ?)`), tunnelerID, nowISO, "tunneler revoked")
		writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
		return
	}
	if len(parts) >= 2 && parts[1] == "grant" {
		...
		_ = state.GrantTunnelerInDB(db, tunnelerID)
		nowISO := isoStringNow()
		_, _ = db.Exec(state.Rebind(`INSERT INTO tunneler_logs (tunneler_id, timestamp, message) VALUES (?, ?, ?)`), tunnelerID, nowISO, "tunneler access granted")
		writeJSON(w, http.StatusOK, map[string]bool{"ok": true})
		return
	}
