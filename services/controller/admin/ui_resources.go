...
func (s *Server) handleUIResources(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case http.MethodGet:
		rows, err := db.Query(`SELECT id, name, type, address, protocol, port_from, port_to, alias, description, remote_network_id, firewall_status FROM resources ORDER BY name ASC`)
		if err != nil {
	...
	case http.MethodPost:
		...
		if _, err := db.Exec(state.Rebind(`INSERT INTO resources (id, name, type, address, ports, protocol, port_from, port_to, alias, description, remote_network_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`),
			id, req.Name, req.Type, req.Address, ports, req.Protocol, nullInt(req.PortFrom), nullInt(req.PortTo), req.Alias, fmt.Sprintf("A new %s resource", strings.ToLower(req.Type)), networkID); err != nil {
